...
def extract(file_path):
    ...
    try:
        ...
        print(f"Extract failed: {file_path} not found.")
        return []
    except json.JSONDecodeError as error:
        print(f"Extract failed: invalid JSON ({error}).")
        return []
...
def validate(data):
    ...
    for record in data:
        ...
        if price > 0 and str(category or '').strip():
            valid_records.append(record)
        else:
            error_count += 1

    print(
        f"Validation summary: {len(valid_records)} valid records, "
